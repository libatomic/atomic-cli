...
func migrateSubstackAction(ctx context.Context, cmd *cli.Command) error {
	...
	dryRun, output, prorate, rewriter, appendMode, _, limit, err := validateMigrateFlags(cmd, requireInstance)
	if err != nil {
		return err
	}
	...
	if isJSONLMode {
		...
		if err := writeImportCSV(subscriberRecords, subscriberOutput, dryRun, prorate, rewriter, appendMode, "substack-stripe", limit); err != nil {
		...
		if len(founderRecords) > 0 {
			if err := writeImportCSV(founderRecords, founderOutput, dryRun, prorate, rewriter, appendMode, "substack-stripe", limit); err != nil {
		...
		if err := writeImportCSV(records, output, dryRun, prorate, rewriter, appendMode, "substack-stripe", limit); err != nil {
